...
- `condition-matrix.js` — cross-condition overview for multi-condition races (network × CPU matrix model with a series per metric, terminal renderer, CSV export of every single result, and the top-level `index.html` with its metric picker and download button)
