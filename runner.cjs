...
async function runBrowserRecording(config, barriers, isParallel, sharedState, opts = {}) {
  const { browserIndex = 0, totalBrowsers = 2, throttle = null, slowmo = 0, noOverlay = false, noRecording = false, ffmpeg = false, har = false, recordingsDir = null, configViewportHeight = null } = opts;
...
async function main() {
  ...
  const { browsers, executionMode, throttle, headless, slowmo, noOverlay, noRecording, ffmpeg, har, recordingsDir, viewportHeight: configViewportHeight } = config;
  const runOpts = { throttle, slowmo, noOverlay, noRecording, ffmpeg, har, recordingsDir, configViewportHeight };
